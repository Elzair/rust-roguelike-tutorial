...
use super::super::super::map::{Map, TileType};
use super::common::{self, MapChunk};
    ...
    for cy in 0..chunks_y {
        for cx in 0..chunks_x {
            ...
            if include_flipping {
                ...
                for y in start_y..end_y {
                    for x in start_x..end_x {
                        let idx = map.xy_idx(end_x - (x + 1), y).unwrap();
                        pattern.push(map.tiles[idx]);
                    }
                }
                patterns.push(pattern);
                ...
                for y in start_y..end_y {
                    for x in start_x..end_x {
                        let idx = map.xy_idx(x, end_y - (y + 1)).unwrap();
                        pattern.push(map.tiles[idx]);
                    }
                }
                patterns.push(pattern);
                ...
                for y in start_y..end_y {
                    for x in start_x..end_x {
                        let idx = map.xy_idx(end_x - (x + 1), end_y - (y + 1)).unwrap();
                        pattern.push(map.tiles[idx]);
                    }
                }
                patterns.push(pattern);
...
pub fn patterns_to_constraints(patterns: Vec<Vec<TileType>>, chunk_size: i32) -> Vec<MapChunk> {
    ...
    for p in patterns {
        ...
        for x in 0..chunk_size {
            ...
            let north_idx = common::tile_idx_in_chunk(chunk_size, x, 0);
            ...
            let south_idx = common::tile_idx_in_chunk(chunk_size, x, chunk_size - 1);
            ...
            let west_idx = common::tile_idx_in_chunk(chunk_size, 0, x);
            ...
            let east_idx = common::tile_idx_in_chunk(chunk_size, chunk_size - 1, x);
...
pub fn render_pattern_to_map(
    ...
    for tile_y in 0..chunk_size {
        for tile_x in 0..chunk_size {
            let map_idx = map.xy_idx(start_x + tile_x, start_y + tile_y).unwrap();
    ...
    for (x, northbound) in chunk.exits[0].iter().enumerate() {
        if *northbound {
            let map_idx = map.xy_idx(start_x + x as i32, start_y).unwrap();
    ...
    for (x, southbound) in chunk.exits[1].iter().enumerate() {
        if *southbound {
            let map_idx = map.xy_idx(start_x + x as i32, start_y + chunk_size-1).unwrap();
            map.tiles[map_idx] = TileType::DownStairs;
        }
    }
    ...
    for (x, westbound) in chunk.exits[2].iter().enumerate() {
        if *westbound {
            let map_idx = map.xy_idx(start_x, start_y + x as i32).unwrap();
            map.tiles[map_idx] = TileType::DownStairs;
        }
    }
    ...
    for (x, eastbound) in chunk.exits[3].iter().enumerate() {
        if *eastbound {
            let map_idx = map.xy_idx(start_x + chunk_size-1, start_y + x as i32).unwrap();
            map.tiles[map_idx] = TileType::DownStairs;
        }
    }
}
