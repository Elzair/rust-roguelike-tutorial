use specs::prelude::*;
use super::components::{CombatStats, DefenseBonus, Equipped, MeleePowerBonus,
     Name, SufferDamage, WantsToMelee};
...
impl<'a> System<'a> for MeleeCombatSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (entities, mut log, mut wants_melee, names, combat_stats, 
            mut inflict_damage, melee_power_bonuses, defense_bonuses,
            equipped) = data;

        for (entity, wants_melee, name, stats) in (&entities, &wants_melee, &names, &combat_stats).join() {
            if stats.hp > 0 {
                let mut total_offensive_bonus = 0;
                for (_item_entity, power_bonus, equipped_by) in (&entities, &melee_power_bonuses, &equipped).join() {
                    if equipped_by.owner == entity {
                        total_offensive_bonus += power_bonus.power;
                    }
                }
                ...
                if target_stats.hp > 0 {
                    ...
                    let damage = i32::max(0, (stats.power+total_offensive_bonus) - (target_stats.defense+total_defense_bonus));
